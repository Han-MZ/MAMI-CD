...
class CDDDataset(CDDataset):
    ...
    def _read_file_paths(self):
        t1_list = sorted(glob(join(self.root, self.subset, 'A', '*.jpg')))
        t2_list = sorted(glob(join(self.root, self.subset, 'B', '*.jpg')))
        tar_list = sorted(glob(join(self.root, self.subset, 'label', '*.jpg')))
        assert len(t1_list) == len(t2_list) == len(tar_list)
        return t1_list, t2_list, tar_list

    def fetch_target(self, target_path):
        return (super().fetch_target(target_path)/255).astype(np.bool_)